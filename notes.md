...
- Symmetric-key cryptography: AKA Secret-key Cryptograpy. A single common key is used for both encryption and decryption processes. Although a single key creates a problem of trnsaferring keys between sender and the reciever securely.
- Asymmetric key cryptography: AKA public key Cryptography. This encryption method uses two keys an encrytion key, named public key and a decryption key, named private key. The key pair generated by this algo consists of these private key and unique public key.  
- Hash functions: This method does not make use of keys while it uses cipher(an algorithm for performing encryption or decryption—a series of well-defined steps that can be followed as a procedure) to generate hash value from plaintext of fixed length. It is almost impossible to recover plain-text from ciphertext.
